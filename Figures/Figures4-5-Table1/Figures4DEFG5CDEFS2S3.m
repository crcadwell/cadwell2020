...
subplot(2,1,1)
X = 1:2;
Y = [0.071740427 0.047173069];
errY = [0.025702623 0.040491051];
scatter(X,Y,100,'r','s','filled'); hold on; 
errorbar(X,Y,errY,'k','Linestyle','none');
ax = gca; ax.XLim = [0.5 2.5]; ax.YLim = [0 0.4]; ax.XTick = 1:2; ax.XTickLabels = {'From L4','From L5'};
ax.YTick = 0:0.2:0.4; ylabel('Fraction of input to L2/3');
plot(ax.XLim,[yu yu],'--k'); plot(ax.XLim, [yu-errYu yu-errYu],'--','Color',[0.5 0.5 0.5]); plot(ax.XLim,[yu+errYu yu+errYu],'--','Color',[0.5 0.5 0.5]);

subplot(2,1,2)
X = 1:2;
Y = [0.186444722 0.191687883];
errY = [0.121855307 0.121558438];
scatter(X,Y,100,'r','s','filled'); hold on; 
errorbar(X,Y,errY,'k','Linestyle','none');
ax = gca; ax.XLim = [0.5 2.5]; ax.YLim = [0 0.4]; ax.XTick = 1:2; ax.XTickLabels = {'From L2/3','From L4'};
ax.YTick = 0:0.2:0.4; ylabel('Fraction of input to L5');
plot(ax.XLim,[yu yu],'--k'); plot(ax.XLim, [yu-errYu yu-errYu],'--','Color',[0.5 0.5 0.5]); plot(ax.XLim,[yu+errYu yu+errYu],'--','Color',[0.5 0.5 0.5]);
...
X = 1:3;
Y = [0.015141204 0.043246943 0.010550808];
errY = [0.01093314 0.015088348 0.01065043];
scatter(X,Y,100,'r','s','filled'); hold on; 
errorbar(X,Y,errY,'k','Linestyle','none');
ax = gca; ax.XLim = [0.5 3.5]; ax.YLim = [0 0.1]; ax.XTick = 1:3; ax.XTickLabels = {'Within L2/3','Within L4','Within L5'};
ax.YTick = 0:0.05:0.1; ylabel('Fraction of lateral input');
plot(ax.XLim,[yu yu],'--k'); plot(ax.XLim, [yu-errYu yu-errYu],'--','Color',[0.5 0.5 0.5]); plot(ax.XLim,[yu+errYu yu+errYu],'--','Color',[0.5 0.5 0.5]);
